...
extends AbstractEval
{
    ...
    {
        STANDARD_HANDLER.registerCommand(aName, aCmd);
    }

    public void registerMacro(String aName, IMacro aMacro)
    {
        MACRO_HANDLER.registerMacro(aName, aMacro);
    }

    public Map<String, ICommand> dumpCommands()
    {
        return STANDARD_HANDLER.dumpCommands();
    }

    public Map<String, IMacro> dumpMacros()
    {
        return MACRO_HANDLER.dumpMacros();
    }
...
implements Eval2.IFrameHandler
{
    private ICommandRepo commands = new BasicCommandRepo();
...
implements Eval2.IFrameHandler
{
    private IMacroRepo macros = new BasicMacroRepo();
    ...
    throws CommandException
    {
        ...
        if(lDataPtr == 0)
        {
            ...
            {
                // Built-in macro call.
                final IMacro lMacro = macros.getMacro((String)lLstExpr.get(0));
                final List lArgs = new ArrayList(lLstExpr.size());
                lArgs.addAll(lLstExpr);
                aStack.push(lMacro.transform(lArgs), lCtx);
            }
            catch(CommandException e)
            {
                Eval2.EvalStack lNestedStack = e.getStack();
