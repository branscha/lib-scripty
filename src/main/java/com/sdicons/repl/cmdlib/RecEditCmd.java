...
extends AbstractCommand
{
	...
	{
		if(aArgs.length != 2 && aArgs.length != 3) throw new CommandException(String.format("ERROR: Bad nr of args. (%s '(<fields>) '(values)).", aArgs[0]));
		if(aArgs[1] == null || !(aArgs[1] instanceof List)) throw new CommandException("ERROR: Expected fields list. (edit '(<fields>) '(values)).");
		List lPropNames = (List) aArgs[1];

		List lPropVals = null;
		if(aArgs.length == 3)
		{
			if(aArgs[2] == null || !(aArgs[2] instanceof List)) throw new CommandException("ERROR: Expected value list. (edit '(<fields>) '(values)).");
			lPropVals = (List) aArgs[2];

			if(lPropNames.size() != lPropVals.size()) throw new CommandException("ERROR: Nr. of property names and values is different.");
		}
		else
		{
			lPropVals = new LinkedList();
			for(int i = 0; i < lPropNames.size(); i++) lPropVals.add("");
		}

		String[] lPropNamesArr = new String[lPropNames.size()];
		Object[] lPropValArr = new Object[lPropVals.size()];

		for(int i = 0; i < lPropNamesArr.length; i++) lPropNamesArr[i] = (String) lPropNames.get(i);
		for(int i = 0; i < lPropValArr.length; i++) lPropValArr[i] = lPropVals.get(i);
