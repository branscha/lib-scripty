...
import com.sdicons.repl.spec.type.StringType;
...
{
    public static enum TeaCmdType {encrypt, decrypt}

    private TeaCmdType type;

    public TeaCmd(TeaCmdType aType)
    {
        type = aType;
    }
    ...
    public static void registerCommands(IRegistry aReg)
    {
        ...
        // Optional parameter
        // - password=... If not provided, the standard IBM EGL password will be used.
        aReg.registerCommand("tea-encrypt", new TeaCmd(TeaCmdType.encrypt));
        // Decrypt a string using TEA.
        // Optional parameter
        // - password=... If not provided, the standard IBM EGL password will be used.
        aReg.registerCommand("tea-decrypt", new TeaCmd(TeaCmdType.decrypt));
    }

    // TYPES
    ////////
    private static final ITypeSpec TSTRING = StringType.STRING_TYPE;
    // ARGS
    ////////
    private static final FixedArg FSTRING = new FixedArg(TSTRING);
    private static final NamedArg NPASSWORD = new NamedArg("password", TSTRING, null, true);
    // PARAMETER LISTS
    //////////////////
    private static final IArgList FSTRING_NPASSWORD_ = new StdArgList(new FixedArg[]{FSTRING}, new OptionalArg[]{}, new NamedArg[]{NPASSWORD});
    ...
    throws CommandException
    {
        try
        {
            switch(type)
            {
                case encrypt:
                {
                    Object[] lArgs = FSTRING_NPASSWORD_.guard(aArgs, aContext);
                    String lPlainText = (String) lArgs[1];
                    String lPwd = (String) lArgs[2];
                    TeaEncrypter lTea;

                    if(lPwd == null) lTea = new TeaEncrypter(); // Standard IBM EGL password.
                    else lTea = new TeaEncrypter(md5(lPwd)); // User provided password.

                    return lTea.encrypt(lPlainText);
                }
                case decrypt:
                {
                    Object[] lArgs = FSTRING_NPASSWORD_.guard(aArgs, aContext);
                    String lCipherText = (String) lArgs[1];
                    String lPwd = (String) lArgs[2];
                    TeaEncrypter lTea;
                    ...
                    return lTea.decrypt(lCipherText);
                }
                default:
    ...
    private byte[] md5(String aString)
    throws CommandException
    {
        try
        {
